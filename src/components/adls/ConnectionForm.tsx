...
import { Database, Key, Lock, LinkIcon, ServerIcon, ShieldCheck, AlertTriangle } from 'lucide-react';
...
const ConnectionForm: React.FC<ConnectionFormProps> = ({ onConnect, isLoading }) => {
  ...
  const fetchAuthMethods = async () => {
    try {
      ...
      // If backend is unavailable, show message and enable mock option
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (backendUnavailable && !useMockBackend) {
  ...
  return (
    <div className="animate-scale-in">
          ...
          {showMockDataOption && (
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => setUseMockBackend(checked === true)}
                className="data-[state=checked]:bg-blue-600 data-[state=checked]:border-blue-600 dark:data-[state=checked]:bg-blue-500 dark:data-[state=checked]:border-blue-500 transition-colors duration-200"
              />
              <Label htmlFor="mock-data" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-blue-900/80 dark:text-blue-100/80">
                Use Mock Data
              </Label>
            </div>
          )}
      ...
      {backendUnavailable && (
        <Alert className="mb-4 bg-amber-50 border-amber-200 dark:bg-amber-900/20 dark:border-amber-700/50">
          <AlertTriangle className="h-5 w-5 text-amber-600 dark:text-amber-500" />
          <AlertDescription className="text-amber-700 dark:text-amber-300">
            Backend server is unavailable. Using mock data for demonstration purposes.
          </AlertDescription>
        </Alert>
      )}
    </div>
